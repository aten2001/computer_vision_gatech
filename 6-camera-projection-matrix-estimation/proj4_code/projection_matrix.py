...
def objective_func(x, **kwargs):
    ...
    #proj = proj.reshape(a*b, -1)
    #points_2d = points_2d.reshape(a*b, -1)
    diff = (proj - points_2d).flatten()
...
def estimate_camera_matrix(pts2d: np.ndarray, 
    ...
    # TODO: YOUR CODE HERE
    ############################################################################
    P = least_squares(objective_func, initial_guess.flatten()[:11], method='lm', verbose=2, max_nfev=50000, kwargs = kwargs)
    #############################################################################
    #                             END OF YOUR CODE
    ############################################################################
    P=P.x
...
def calculate_camera_center(P: np.ndarray,
    ...
    # TODO: YOUR CODE HERE
    ############################################################################
    cc = -np.dot(np.dot(inv(R_T),inv(K)),P)
    cc = cc[:,3]
    #############################################################################
    #                             END OF YOUR CODE
    ############################################################################
